...
const LS_KEY = "vendorReadNotifications";
...
export default function VendorNotificationDropdown() {
  ...
  const handleNotificationClick = (id) => {
    ...
    if (!readNotifications.includes(id)) {
      localStorage.setItem(LS_KEY, JSON.stringify([...readNotifications, id]));
      refresh();
    }
    setIsOpen(false);
  };
  ...
  const getIcon = (type) => {
    switch (type) {
      case "ORDER":
        return <FiPackage className="text-purple-500 bg-purple-100 p-2 rounded-full w-8 h-8 shrink-0" />;
      case "OFFER":
        return <FiTag className="text-orange-500 bg-orange-100 p-2 rounded-full w-8 h-8 shrink-0" />;
      case "MESSAGE":
        return <FiMessageSquare className="text-green-500 bg-green-100 p-2 rounded-full w-8 h-8 shrink-0" />;
      default:
        return <FiBell className="text-gray-500 bg-gray-100 p-2 rounded-full w-8 h-8 shrink-0" />;
    }
  };
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className="p-2 rounded-xl bg-gray-300 hover:bg-purple-600 group transition-colors duration-200 relative flex items-center justify-center outline-none"
      >
      ...
      {/* Dropdown Menu */}
      <div
        className={`absolute right-0 mt-3 w-80 bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 overflow-hidden transition-all duration-200 origin-top-right ${
          isOpen
              ...
              {notifications.map((notif) => {
                ...
                return (
                  <Link
                    ...
                    className={`p-4 border-b border-gray-50 flex gap-3 hover:bg-purple-50 transition-colors duration-200 ${
